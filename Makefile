...
compute-unit-test compute-e2e-test compute-fusion-test tpu-test quant-test rope-test vpu1-test vpu1-tests norm-test norm-distributed-test: export MAKEFLAGS += VK_PCH_I_FAST= VK_PCH_I_SLOW=
compute-unit-test:
...
compute-fusion-test:
	$(RTL_SBT) $(COMPUTE_SOURCES) \
	  'set Test / unmanagedSources ~= (_.filter(_.getName == "ComputeUnit_E2E_Test.scala"))' \
	  'testOnly npu.top.ComputeUnitE2ETest -- -z "fuse TPU GEMM"'
